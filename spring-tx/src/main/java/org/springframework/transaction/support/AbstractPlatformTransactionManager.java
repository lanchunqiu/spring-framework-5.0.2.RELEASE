...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
		...
		//判断当前线程是否存在事务，判断依据为当前线程记录的连接不为空且连接中transactionActive属性不为空
	...
	private TransactionStatus handleExistingTransaction(
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NEVER) {
			//如果事务的传播属性为PROPAGATION_NEVER，则抛异常
			throw new IllegalTransactionStateException(
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NOT_SUPPORTED) {
			...
			//如果事务的传播属性为PROPAGATION_NOT_SUPPORTED，则挂起原事务
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRES_NEW) {
			...
			//如果事务的传播属性为PROPAGATION_REQUIRES_NEW，则先挂起原事务然后建立新事务
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
			...
			if (useSavepointForNestedTransaction()) {
				...
				//如果没有可以使用保存点的方式控制事务回滚，那么在嵌入式事务建立时创建保存点
				...
				//有些情况是不能使用保存点操作的，比如JTA，那么就建立新事务
				boolean newSynchronization = (getTransactionSynchronization() != SYNCHRONIZATION_NEVER);
				DefaultTransactionStatus status = newTransactionStatus(
	...
	public final void commit(TransactionStatus status) throws TransactionException {
		...
		//如果在事务量中已经被标记回滚，那么不会尝试提交事务，直接回滚
